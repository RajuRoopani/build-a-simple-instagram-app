 ...
 * Pure vanilla JS, IIFE-wrapped, no ES modules, no build step.
 * Sections: 1.MOCK_DATA 2.State 3.Helpers 4.Render 5.Handlers 6.Stories 7.NewPost 8.Toast 9.Init
 */
(function () {
  'use strict';
  ...
  var state = {
    ...
    storyTimer: null, storyIndex: -1,
    toastTimer: null,
    tapTimer: null, lastTapWrap: null
  };
  ...
  function userById(id) {
    ...
    for (var i = 0; i < MOCK_DATA.users.length; i++) {
      if (MOCK_DATA.users[i].user_id === id) return MOCK_DATA.users[i];
    }
    return null;
  }
  ...
  function renderStories() {
    ...
    MOCK_DATA.stories.forEach(function (s, i) {
      var u = userById(s.user_id);
      if (!u) return;
      var ringCls = s.seen ? 'story-ring story-ring--seen' : 'story-ring';
      h += '<div class="story-item" data-story-index="' + i + '" role="listitem" tabindex="0">'
         + '<div class="' + ringCls + '">'
         + '<img src="' + esc(u.profile_picture_url) + '" alt="' + esc(u.username) + '">'
         + '</div>'
  ...
  function buildPostCard(post) {
    ...
    if (post.user_id !== MOCK_DATA.currentUser.user_id) {
      folBtn = '<button class="follow-btn' + (fol ? ' following' : '') + '"'
             + ' data-action="follow" data-user-id="' + post.user_id + '">'
             + (fol ? 'Following' : 'Follow') + '</button>';
    }
    var cmtHtml = '';
    cs.slice(0, 2).forEach(function (c) {
      var cu = userById(c.user_id);
      cmtHtml += '<p class="post-comment"><strong>'
               + esc(cu ? cu.username : 'user') + '</strong> ' + esc(c.text) + '</p>';
    });
    if (cs.length > 2) {
      cmtHtml += '<p class="view-all-comments">View all ' + cs.length + ' comments</p>';
    }
    return '<article class="post-card" data-post-id="' + post.post_id + '">'
      + '<header class="post-header">'
      + '<img class="post-avatar" src="' + esc(u.profile_picture_url) + '" alt="">'
      + '<div class="post-user-info">'
      + '<span class="post-username">' + esc(u.username) + '</span>'
      ...
      + '</div>'
      + '<button class="action-btn bookmark-btn" data-action="bookmark" data-post-id="' + post.post_id + '" aria-label="Bookmark">' + bmIco + '</button>'
      + '</div>'
  ...
  function renderFeed() {
    var f = document.getElementById('postFeed');
    if (!f) return;
    var h = '';
    MOCK_DATA.posts.forEach(function (p) { h += buildPostCard(p); });
    f.innerHTML = h;
  }
  ...
  function renderSidebarProfile() {
    ...
    el.innerHTML = '<img class="sidebar-avatar" src="' + esc(cu.profile_picture_url) + '" alt="">'
      ...
      + '<span class="sidebar-username">' + esc(cu.username) + '</span>'
      + '<span class="sidebar-name">' + esc(cu.display_name) + '</span>'
      + '</div>';
  }
  ...
  function renderSuggestions() {
    ...
    MOCK_DATA.suggestions.forEach(function (uid) {
      ...
      h += '<div class="suggestion-item">'
         ...
         + '<span class="suggestion-username">' + esc(u.username) + '</span>'
         + '<span class="suggestion-subtitle">Suggested for you</span>'
         + '</div>'
         + '<button class="follow-btn suggestion-follow' + (fol ? ' following' : '') + '"'
         + ' data-action="follow" data-user-id="' + uid + '">'
         + (fol ? 'Following' : 'Follow') + '</button>'
         + '</div>';
    });
    el.innerHTML = h;
  ...
  function toggleLike(pid) {
    if (state.likedPosts[pid]) {
      delete state.likedPosts[pid];
      state.likeCounts[pid] = (state.likeCounts[pid] || 1) - 1;
    } else {
      state.likedPosts[pid] = true;
      state.likeCounts[pid] = (state.likeCounts[pid] || 0) + 1;
    }
    renderFeed();
  }

  function triggerHeartBurst(wrap, pid) {
    var burst = wrap.querySelector('.heart-burst');
    if (burst) {
  ...
  function toggleBookmark(pid) {
    if (state.bookmarkedPosts[pid]) delete state.bookmarkedPosts[pid];
    else state.bookmarkedPosts[pid] = true;
    renderFeed();
  }
  ...
  function toggleFollow(uid) {
    if (state.followedUsers[uid]) delete state.followedUsers[uid];
    else state.followedUsers[uid] = true;
    renderFeed();
    renderSuggestions();
  }
  ...
  function startStoryProgress() {
    ...
    if (_progBar) {
      ...
      _progBar.style.transition = 'width 5s linear';
      _progBar.style.width = '100%';
    }
    state.storyTimer = setTimeout(function () {
      var next = state.storyIndex + 1;
      if (next < MOCK_DATA.stories.length) openStory(next);
      else closeStory();
    }, 5000);
  }
  ...
  function openStory(idx) {
    ...
    if (av)  av.src = u.profile_picture_url;
  ...
  function handleNewPost() {
    var cap = prompt("What's on your mind?");
    if (!cap || !cap.trim()) return;
    var newId = 'p' + Date.now();
    var np = {
      post_id:     newId,
      user_id:     MOCK_DATA.currentUser.user_id,
      like_count:  0,
      caption:     cap.trim(),
      content_url: 'https://picsum.photos/seed/' + newId + '/600/600',
      created_at:  new Date().toISOString()
    };
    MOCK_DATA.posts.unshift(np);
    state.likeCounts[newId] = 0;
    renderFeed();
  ...
  function bindEvents() {
    ...
    if (feed) {
      ...
      feed.addEventListener('click', function (e) {
        var wrap = e.target.closest('.post-image-wrap');
        if (!wrap) return;
        var pid = wrap.dataset.postId;
        if (!pid) return;
        if (state.tapTimer && state.lastTapWrap === wrap) {
          clearTimeout(state.tapTimer);
          state.tapTimer = null; state.lastTapWrap = null;
    ...
    if (bar) {
      bar.addEventListener('click', function (e) {
        var item = e.target.closest('[data-story-index]');
        if (!item) return;
        var idx = parseInt(item.dataset.storyIndex, 10);
        if (!isNaN(idx)) openStory(idx);
      });
    ...
    if (_overlay) {
      _overlay.addEventListener('click', function (e) {
        if (e.target === _overlay) closeStory();
      });
    }
...
with open('/workspace/instagram_app/static/app.js', 'w') as f:
    f.write(content)

with open('/workspace/instagram_app/static/app.js', 'r') as f:
    lines = f.readlines()
print(f"Written: {len(lines)} lines, {sum(len(l) for l in lines)} bytes")
